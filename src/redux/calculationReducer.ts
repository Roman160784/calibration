...
const slice = createSlice({
    ...
    reducers: {
        addCalibrationFieldAC(state, action: PayloadAction<{id: string, dot: number, array?: number[]}>,){

            
            
            let dataForCalibration = new Array(10);
            
            if(action.payload.array === undefined) {
                dataForCalibration.fill(action.payload.dot)
            }else{
...
export const {addCalibrationFieldAC, } = slice.actions
